...
#include <math.h> 
#include "matrix.h"
/*Solve Lagrangian-Relaxation Problem by Newton Method
多元函数[高/二次]-使用牛顿法求解拉格朗日松弛 
Aouthor:Xipnig.Yu
Date:2020.05.28 
------------ 
min F(X)
...
//	MATRIX_TYPE _mat_X [1][2] = {0,0}; //Two-dimensional function
...
s.t. x_1+x_2=1
---------- */  
double Function(Matirx* mat_X){/*成功使用拉格朗日松弛法求解
[问题]
F(x)=x_1^2+x_2^2
s.t. x_1+x_2=1*/ 
	/*Y=XAX'+BX+C*/
	MATRIX_TYPE Lagrange = 1;
	/*mat_A*/ 
//	MATRIX_TYPE _mat_A [2][2] = {1,0,0,1}; //Two-dimensional function
	MATRIX_TYPE _mat_A [3][3] = {1,0,1,0,1,1,0,0,0}; //Three-dimensional function 
	/*-具体生成A-*/
	...
	/*mat_B*/
//	MATRIX_TYPE _mat_B [2][1] = {1,-1}; //Two-dimensional function
	MATRIX_TYPE _mat_B [3][1] = {0,0,1}; //Three-dimensional function 
	/*-具体生成B-*/
	...
	M_free(mat_temp);
	/*XAX'+BX + C*/ 
	double Result = (mat_Result->data)[0] + 0;//pow(Lagrange,2); 
    return Result;
}
...
同时出现了不可行解的问题
---------- */ 
//double Function(Matirx* mat_X){/*拉格朗日函数的解并非可行解，需要使用增广拉格朗日函数*/
///*无法使用拉格朗日松弛法求解，一方面因为函数结构错误了；同时出现了不可行解的问题*/ 
//	/*Y=XAX'+BX+C*/
//	MATRIX_TYPE Lagrange = 1;
//	/*mat_A*/ 
//	MATRIX_TYPE _mat_A [2][2] = {1,0,0,1}; //Two-dimensional function
...
//	Matirx*  mat_B = Matrix_gen(row,column,_mat_B);
//	/*XAX'+BX*/
//	Matirx* mat_Result = M_add_sub(1,M_mul(M_mul(M_mul(mat_X,mat_A),M_T(mat_X)),M_T(M_mul(M_mul(mat_X,mat_A),M_T(mat_X)))),2,M_mul(M_mul(mat_X,mat_A),M_T(mat_X)));
//	Matirx*  mat_temp = mat_Result;
//	mat_Result = M_add_sub(Lagrange,mat_temp,-1,M_mul(mat_X,mat_B));
//	M_free(mat_temp);
//	/*XAX'+BX + C*/ 
//	double Result = (mat_Result->data)[0] + pow(Lagrange,2); 
//    return Result;
//}
...
//	/*mat_A*/ 
////	MATRIX_TYPE _mat_A [2][2] = {1,0,0,1}; //Two-dimensional function
//	MATRIX_TYPE _mat_A [3][3] = {1,0,0,0,-1,1,0,0,0}; //Three-dimensional function 
//	/*-具体生成A-*/
//	int row = sizeof(_mat_A) / sizeof(_mat_A[0]);
//	int column = sizeof(_mat_A[0]) / sizeof(_mat_A[0][0]);
//	Matirx*  mat_A = Matrix_gen(row,column,_mat_A);
//	/*mat_B*/
////	MATRIX_TYPE _mat_B [2][1] = {1,-1}; //Two-dimensional function
//	MATRIX_TYPE _mat_B [3][1] = {0,-3,0}; //Three-dimensional function 
//	/*-具体生成B-*/
//	row = sizeof(_mat_B) / sizeof(_mat_B[0]);
//	column = sizeof(_mat_B[0]) / sizeof(_mat_B[0][0]);
//	Matirx*  mat_B = Matrix_gen(row,column,_mat_B);
